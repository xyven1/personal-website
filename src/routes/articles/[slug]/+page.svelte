<script lang="ts">
	...
	export let data;
	if (data.meta.history.length < 1) throw Error('Article must have history');
	...
	function dateToDateString(date: string) {
		return new Date(date).toLocaleDateString('en-US', {
			timeZone: 'EST',
			dateStyle
		});
	}
	function dateToDateTimeString(date: string) {
...
<article class="max-w-full py-8">
	<section class="border-b-[3px] border-neutral-800 pb-2">
		...
		<p class="flex flex-wrap gap-x-2 text-stone-600 dark:text-neutral-400">
			<span>{dateToDateString(data.meta.date)}</span>
			{#if data.meta.updated}
				<span>(<i>Updated: {dateToDateString(data.meta.updated)}</i>)</span>
			{/if}
				...
				<div class="overflow-hidden">
					<ol class="px-1">
						{#each data.meta.history as commit}
							...
							>
								<div>
									{dateToDateTimeString(commit.date)} - {commit.message}
								</div>
								<div class="space-x-1">
									{#if commit.liveUrl !== ''}
